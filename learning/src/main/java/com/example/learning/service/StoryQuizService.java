...
public class StoryQuizService {
    ...
    public SubmitResponseDTO submitBaseQuiz(Long userId, Long storyId, List<AnswerDTO> answers) {
        ...
        for (AnswerDTO answer : answers) {
            StoryQuizQuestionEntity q = baseQuestions.stream()
                    .filter(qq -> qq.getId().equals(answer.getQuestionId()))
                    .findFirst().orElse(null);
            if (q != null && q.getCorrectAnswer().equals(answer.getAnswer())) {
                correctCount++;
                switch (q.getDifficulty()) {
                    case LOW -> baseScore += 1;
                    case MEDIUM -> baseScore += 2;
                    case HIGH -> baseScore += 3;
                }
            }
        }

        Difficulty baseDifficulty = baseQuestions.get(0).getDifficulty();
        ...
        boolean hasBonus = (baseDifficulty != Difficulty.HIGH) && (correctCount == baseQuestions.size());
        Difficulty nextDifficulty = null;
        if (hasBonus) {
            nextDifficulty = (baseDifficulty == Difficulty.LOW) ? Difficulty.MEDIUM
                    : (baseDifficulty == Difficulty.MEDIUM) ? Difficulty.HIGH : null;
        }
    ...
    public SubmitResponseDTO submitBonusQuiz(Long userId, Long storyId, Difficulty bonusDifficulty,
        ...
        for (AnswerDTO answer : answers) {
            StoryQuizQuestionEntity q = bonusQuestions.stream()
                    .filter(qq -> qq.getId().equals(answer.getQuestionId()))
                    .findFirst().orElse(null);
            if (q != null && q.getCorrectAnswer().equals(answer.getAnswer())) {
                bonusCorrectCount++;
                switch (q.getDifficulty()) {
                    case LOW -> bonusScore += 1;
                    case MEDIUM -> bonusScore += 2;
                    case HIGH -> bonusScore += 3;
                }
            }
        }

        // 기본 문제 점수 계산 (보너스 난이도 기준)
        int baseScore = (bonusDifficulty == Difficulty.MEDIUM) ? 5
                : (bonusDifficulty == Difficulty.HIGH) ? 10 : record.getTotalScore();

        // 기존 record 업데이트
        record.setBonusCorrectCount(bonusCorrectCount);
        record.setTotalScore(baseScore + bonusScore);
