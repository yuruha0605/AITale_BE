...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    @Value("${app.oauth2.redirect-url:http://localhost:3000/login}")
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        redisTemplate.opsForValue()
                .set("RT:" + userEntity.getUserSystemId(), rt, REFRESH_TOKEN_TTL, TimeUnit.SECONDS);

        // 4. 리다이렉트 처리 (Map을 리턴하는 대신 URL에 담아서 보냄)
        String targetUrl = UriComponentsBuilder.fromUriString(oauthRedirectUrl)
