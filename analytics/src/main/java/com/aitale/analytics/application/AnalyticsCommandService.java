...
public class AnalyticsCommandService {
  ...
  private void updateDailyScoreStat(StudyResultCreateRequest request) {
    ...
    dailyScoreStatRepository
        .findByUserIdAndStatDate(request.userId(), statDate)
        .ifPresentOrElse(
            stat -> {
              ...
              double newAvgScore = newQuizCount == 0 ? 0.0 : (double) newTotalScoreSum / newQuizCount;

              stat.updateStats(
                  newTotalScoreSum, newQuizCount, newCorrectCount, newWrongCount, newAvgScore, now);
            },
            () -> {
  ...
  private void updateUserGrowthSnapshot(StudyResultCreateRequest request) {
    ...
    userGrowthSnapshotRepository
        .findByUserIdAndSnapshotDate(request.userId(), snapshotDate)
        .ifPresentOrElse(
            snapshot -> {
              ...
              double newOverallCorrectRate = newTotalQuizCount == 0 ? 0.0
    ...
    List<UserGrowthSnapshot> futureSnapshots = userGrowthSnapshotRepository
        .findByUserIdAndSnapshotDateGreaterThanOrderBySnapshotDateAsc(
            request.userId(), snapshotDate);
  ...
  private void createSnapshotFromPrevious(
      StudyResultCreateRequest request, LocalDate snapshotDate, LocalDateTime now) {
    ...
    int baseTotalQuizCount = previousSnapshot == null ? 0 : previousSnapshot.getTotalQuizCount();
    int baseTotalCorrectCount = previousSnapshot == null ? 0 : previousSnapshot.getTotalCorrectCount();
    ...
    double newOverallCorrectRate = newTotalQuizCount == 0 ? 0.0 : (double) newTotalCorrectCount / newTotalQuizCount;
  ...
  private void updateGenrePerformanceStat(StudyResultCreateRequest request) {
    ...
    genrePerformanceStatRepository
        .findByUserIdAndGenre(request.userId(), request.genre())
        .ifPresentOrElse(
            stat -> {
              ...
              double newCorrectRate = newAttemptCount == 0 ? 0.0 : newTotalCorrect / newAttemptCount;
              double newAvgScore = newAttemptCount == 0 ? 0.0 : newTotalScore / newAttemptCount;

              stat.updateStats(
                  newAttemptCount, newCorrectRate, newAvgScore, request.solvedAt(), now);
            },
            () -> {
